...
# Start HTTP server for dashboard (port 8765)
python3 -m http.server 8765 --directory docs &
PID_HTTP=$!
echo "Dashboard: http://localhost:8765/trading.html"
...
cleanup() {
    echo ""
    echo "Shutting down..."
    kill $PID_HTTP $PID_PREDICT $PID_MONITOR $PID_RENDER 2>/dev/null || true
    exit 0
}
